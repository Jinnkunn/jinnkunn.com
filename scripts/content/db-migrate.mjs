...
//
// Usage:
//   node scripts/content/db-migrate.mjs --local                    # local D1 (dev)
//   node scripts/content/db-migrate.mjs --remote --env=staging
//   node scripts/content/db-migrate.mjs --remote --env=production
//
...
function parseArgs() {
  ...
  for (const arg of process.argv.slice(2)) {
    if (arg.startsWith("--env=")) passthrough.push(arg);
      ...
      console.error(
        "usage: node scripts/content/db-migrate.mjs [--local|--remote] [--env=staging|production]",
      );
      process.exit(2);
    }
  }
