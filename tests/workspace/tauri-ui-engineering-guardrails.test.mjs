...
test("tauri-ui-engineering: root package exposes workspace UI smoke", async () => {
  ...
  assert.equal(pkg.scripts["check:workspace-ui"], "node scripts/qa/workspace-ui-smoke.mjs");
  assert.equal(
    pkg.scripts["qa:workspace:site-admin"],
    "node scripts/qa/workspace-site-admin-qa.mjs",
  );
  assert.equal(
    pkg.scripts["qa:workspace:site-admin:local"],
    "node scripts/qa/workspace-site-admin-qa.mjs --local-only",
  );
  assert.equal(
    pkg.scripts["qa:workspace:site-admin:staging"],
    "node scripts/qa/workspace-site-admin-qa.mjs --skip-workspace-build --skip-workspace-tests --skip-workspace-ui",
  );
});
...
test("tauri-ui-engineering: publish surfaces stale staging candidates as a rebuild step", async () => {
  ...
  assert.match(releasePanel, /Technical Status/);
  assert.match(releasePanel, /Draft to Live/);
  ...
  assert.match(releasePanel, /Live behind Draft/);
  ...
  assert.match(releasePanel, /release:status:json/);
  ...
  assert.match(statusPanel, /NowStatusComposer/);
  ...
  assert.match(releasePanel, /Publishing starts from Draft/);
  assert.match(releasePanel, /Release gates/);
  assert.match(releasePanel, /Draft \/ Live route/);
  assert.match(releasePanel, /Live differs/);
  assert.match(releasePanel, /GitHub Dispatch Fallback/);
  assert.match(releaseHistoryPanel, /Release History/);
  assert.match(releaseFlow, /Publish Draft to Live/);
