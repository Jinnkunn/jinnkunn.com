...
test("cloudflare deploy env: branch defaults map staging/prod target", (t) => {
  withEnv(t, {
    CLOUDFLARE_DEPLOY_ENV: null,
    DEPLOY_ENV: null,
    SITE_ADMIN_REPO_BRANCH_STAGING: null,
    SITE_ADMIN_REPO_BRANCH_PRODUCTION: null,
    SITE_ADMIN_REPO_BRANCH: "site-admin-staging",
  });
  assert.equal(resolveCloudflareTargetEnv(process.env), "staging");

  process.env.SITE_ADMIN_REPO_BRANCH = "main";
  assert.equal(resolveCloudflareTargetEnv(process.env), "production");
});

test("cloudflare deploy env: explicit deploy env overrides branch inference", (t) => {
  withEnv(t, {
