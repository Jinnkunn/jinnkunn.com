...
import {
  loadSiteAdminHomeData,
  normalizeHomeData,
  saveSiteAdminHomeData,
} from "@/lib/server/site-admin-home-service";
...
const RATE_LIMIT = { namespace: "site-admin-home" };
...
export async function GET(req: NextRequest) {
  return withSiteAdminContext(
    ...
    async () => {
      try {
        const { data, sourceVersion } = await loadSiteAdminHomeData();
        return apiPayloadOk<Omit<SiteAdminHomeGetPayload, "ok">>({
          data,
          sourceVersion,
        });
      } catch (err: unknown) {
...
function parsePatchCommand(raw: Record<string, unknown>):
  ...
  const data = normalizeHomeData(dataRaw);
...
export async function POST(req: NextRequest) {
  return withSiteAdminContext(
    ...
    async (ctx) => {
      ...
      try {
        const result = await saveSiteAdminHomeData({ data, expectedFileSha });
        await writeSiteAdminAuditLog({
          actor: ctx.login,
          action: "home.save",
          endpoint: "/api/site-admin/home",
          method: "POST",
          status: 200,
          result: "success",
          code: "OK",
