...
import {
  loadSiteAdminPublicationsData,
  saveSiteAdminPublicationsData,
  normalizePublicationsData,
} from "@/lib/server/site-admin-publications-service";
...
const RATE_LIMIT = { namespace: "site-admin-publications" };
...
export async function GET(req: NextRequest) {
  return withSiteAdminContext(
    ...
    async () => {
      try {
        const { data, sourceVersion } = await loadSiteAdminPublicationsData();
        return apiPayloadOk<Omit<SiteAdminPublicationsGetPayload, "ok">>({
          data,
          sourceVersion,
        });
      } catch (err: unknown) {
...
function parsePatchCommand(raw: Record<string, unknown>):
  ...
  const data = normalizePublicationsData(dataRaw);
...
export async function POST(req: NextRequest) {
  return withSiteAdminContext(
    ...
    async (ctx) => {
      ...
      try {
        const result = await saveSiteAdminPublicationsData({
          data,
          expectedFileSha,
        });
        await writeSiteAdminAuditLog({
          actor: ctx.login,
          action: "publications.save",
          endpoint: "/api/site-admin/publications",
          method: "POST",
          status: 200,
          result: "success",
          code: "OK",
          ...
          metadata: {
            ...
            profileLinkCount: data.profileLinks.length,
          },
        });
