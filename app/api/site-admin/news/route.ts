...
import {
  loadSiteAdminNewsData,
  normalizeNewsData,
  saveSiteAdminNewsData,
} from "@/lib/server/site-admin-news-service";
import { isSiteAdminSourceConflictError } from "@/lib/server/site-admin-source-store";
import type {
  SiteAdminNewsData,
  SiteAdminNewsGetPayload,
} from "@/lib/site-admin/api-types";

export const runtime = "nodejs";

const RATE_LIMIT = { namespace: "site-admin-news" };
...
export async function GET(req: NextRequest) {
  return withSiteAdminContext(
    ...
    async () => {
      try {
        const { data, sourceVersion } = await loadSiteAdminNewsData();
        return apiPayloadOk<Omit<SiteAdminNewsGetPayload, "ok">>({
          data,
          sourceVersion,
        });
      } catch (err: unknown) {
...
function parsePatchCommand(raw: Record<string, unknown>):
  ...
  const data = normalizeNewsData(dataRaw);
...
export async function POST(req: NextRequest) {
  return withSiteAdminContext(
    ...
    async (ctx) => {
      ...
      try {
        const result = await saveSiteAdminNewsData({ data, expectedFileSha });
        await writeSiteAdminAuditLog({
          actor: ctx.login,
          action: "news.save",
          endpoint: "/api/site-admin/news",
          method: "POST",
          status: 200,
          result: "success",
          code: "OK",
