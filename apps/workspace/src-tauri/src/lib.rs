...
#[cfg(any(target_os = "ios", target_os = "android"))]
fn toggle_main_window(_app: &tauri::AppHandle) {}
...
#[cfg(any(target_os = "ios", target_os = "android"))]
fn install_tray(_app: &tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    Ok(())
}

#[cfg(not(any(target_os = "ios", target_os = "android")))]
fn install_desktop_shell_plugins(
...
#[cfg(not(any(target_os = "ios", target_os = "android")))]
fn run_tauri_app(app: tauri::App<tauri::Wry>) {
...
pub fn run() {
    ...
    let builder = install_desktop_shell_plugins(builder)
    ...
    let builder = install_menu_event_handler(builder)
        .setup(|app| {
