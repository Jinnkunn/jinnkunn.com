...
export async function postSiteAdminDeployBackend():
  ...
  try {
    ...
    // In db mode the fresh bundle must be built locally from staging D1.
    // Do not silently dispatch GitHub Actions here; that path is now an
    // explicit fallback link in the workspace UI.
    if (sourceState?.storeKind === "db") {
      return backendError(
        "Local Cloudflare release required. Run `npm run release:staging` from the Tauri workspace or this repo checkout; GitHub Actions fallback is manual-only.",
        409,
        "LOCAL_RELEASE_REQUIRED",
      );
    }
