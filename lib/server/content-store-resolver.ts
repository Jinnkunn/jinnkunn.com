...
// and doesn't pull in the GitHub or D1 clients when they're not needed.
...
function tryGetD1Executor(): DbExecutor | null {
  ...
  // build, in scripts). Fall back to local in those cases — same forgiving
  // behavior the GitHub backend uses for missing env.
  try {
