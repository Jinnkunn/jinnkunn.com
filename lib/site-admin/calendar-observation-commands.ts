...
// Keep this aligned with the desktop resident-agent/Rust collector contract:
// anything accepted during POST sync must remain addressable by cleanup.
...
/**
 * Parse the narrow collector-cleanup command accepted by
 * DELETE /api/site-admin/calendar-observations.
 *
 * The command deliberately cannot name sources, observations, entities, or
 * arbitrary SQL-like filters: the authenticated caller may only tombstone one
 * collector, optionally within one explicit time window.
 */
